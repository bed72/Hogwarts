...
class PasswordValidatorLiveData(private val validatorUseCase: ValidatorUseCase) {
    private val _isValid = MutableLiveData<ValidateFormEntity>()
    val isValid: LiveData<ValidateFormEntity> get() = _isValid

    fun set(value: String) {
        if (value.isNotEmpty()) _isValid.value = validatorUseCase.validatePassword(value)
    }
}
